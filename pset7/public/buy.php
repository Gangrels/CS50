<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        ...
        if(preg_match("/^\d+$/", $_POST["shares"])==true )
        {
         ...
         {
            $submit= CS50::query("INSERT INTO portfolios (user_id, symbol, shares) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE shares = shares + VALUES(shares)", $_SESSION["id"], $stock["symbol"], $shares);
             if($submit=false)
             {
